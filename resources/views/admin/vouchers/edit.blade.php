                    ...
                    <form action="{{ route('admin.vouchers.update', $voucher) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                    ...
                                    name="description" 
                                    rows="3" 
                                    placeholder="Mô tả voucher">{{ old('description', $voucher->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror" 
                                        ...
                                        <option value="product" {{ old('type', $voucher->type) == 'product' ? 'selected' : '' }}>
                                            Giảm giá sản phẩm
                                        </option>
                                    </select>
                                    @error('type')
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" 
                                            ...
                                            <option value="fixed" {{ old('discount_type', $voucher->discount_type ?? 'fixed') == 'fixed' ? 'selected' : '' }}>đ</option>
                                            <option value="percentage" {{ old('discount_type', $voucher->discount_type ?? 'fixed') == 'percentage' ? 'selected' : '' }}>%</option>
                                        </select>
                                    </div>
                        ...
                        <div class="alert alert-info">
                            <h6 class="alert-heading">Thống kê sử dụng</h6>
                            <p class="mb-1">Đã sử dụng: <strong>{{ $voucher->used_count }}</strong> lần</p>
                            @if($voucher->max_usage)
                                <p class="mb-0">Giới hạn: <strong>{{ $voucher->max_usage }}</strong> lần</p>
                            @else
                                <p class="mb-0">Không giới hạn số lần sử dụng</p>
                            @endif
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Khởi tạo dựa trên các giá trị hiện tại
    ...
    // Xử lý sự kiện click nút tạo mã voucher
