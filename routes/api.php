<?php

use App\Http\Controllers\Api\CustomersReturnController;
use App\Http\Controllers\Api\OrderSuccessController;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/
Route::post('/chart', [\App\Http\Controllers\Api\ChartController::class, 'getDataToChart'])->name('api.chart'); // Route for chart data
Route::get('/order',[CustomersReturnController::class,'getCustomerReturnRateComparison'])->name('api.conversion');
Route::middleware('auth:sanctum')->get('/user', function (Request $request) {
    return $request->user();
});